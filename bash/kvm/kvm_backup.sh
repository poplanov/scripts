...
data=`date +%Y-%m-%d`
# Папка для бэкапов
backup_dir=/DATA/backup/current
...
#vm_list=(vm-1 vm-2)
# Лог файл
logfile="/DATA/backup/current/kvmbackup.log"
echo -e "\n\n-------START BACKUP $data---------" >> $logfile
    ...
    do
        ...
        for disk in $disk_list
            do
                ...
                snap_path=`virsh -c qemu:///system domblklist "$activevm" | grep $disk | awk '{print $2}' | grep snapshot`
...
for offvm in $offvm_list
    do
        ...
        # Бэкапим конфигурацию
        virsh -c qemu:///system dumpxml "$offvm" > $backup_dir/$offvm/$offvm-$data.xml
